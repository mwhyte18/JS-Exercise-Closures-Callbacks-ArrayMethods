...
function processDuplicateFree(list, callback) {
  ...
  return callback(list.filter(function(item) {
    return dupFree.hasOwnProperty(item) ? false : (dupFree[item] = true);
  }));
...
function getFullNames(runners) {
  ...
  runners.forEach(function(element, index){
    runnersNames.push(`${element.last_name}, ${element.first_name}`)
  })
  return runnersNames;
}
...
function firstNamesAllCaps(runners) {
  const firstNameCaps = runners.map(function(item) { return item["first_name"].toUpperCase();});
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const tShirtRunners = runners.filter(function(e){
    return e.shirt_size == tShirtSize;
  })
  return tShirtRunners;
}
...
function tallyUpDonations(runners) {
  const totalDonos = runners.reduce(function(prev, cur){
